			...
			<%@ page import="java.util.List" %>
				<%@ page import="br.edu.exemplo.dao.BrinquedoDAO" %>
					...
					<head>
						...
						<title>Categoria | Toylandia</title>
					</head>
					...
					<body style="background-color: var(--complement1);">
							...
							<div>
								<ul class="flex space-x-6">
									...
									<li class="flex gap-2 justify-center items-center">
										<a class="cursor-pointer links nav-link font-semibold"
											id="categoriasLink">Categorias</a>
										<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
											stroke-width="2" stroke="currentColor"
											class="links cursor-pointer text-purple-800 w-6 h-6" id="categoriasIcon">
											<path stroke-linecap="round" stroke-linejoin="round"
												d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
										</svg>

										<% BrinquedoDAO daoCategoria=new BrinquedoDAO(); List<String> categorias =
											daoCategoria.listarCategorias();
											%>

											<div class="top-14 absolute hidden bg-white text-gray-800 border border-purple-500 mt-2 py-2 w-48 rounded shadow-lg"
												id="dropdownContent">
												<ul>
													<a href="ServletBrinquedo?cmd=listarCategoria&categoria=Jogos"
														class="px-4 py-2 block w-full links cursor-pointer">Jogos</a>
													<a href="ServletBrinquedo?cmd=listarCategoria&categoria=Bonecos"
														class="px-4 py-2 block w-full links cursor-pointer">Bonecos</a>
													<a href="ServletBrinquedo?cmd=listarCategoria&categoria=Bebes"
														class="px-4 py-2 block w-full links cursor-pointer">Bebes</a>
													<a href="ServletBrinquedo?cmd=listarCategoria&categoria=Esportes"
														class="px-4 py-2 block w-full links cursor-pointer">Esportes</a>

													<% for (String categoria : categorias) { %>
														<li>
															<a href="ServletBrinquedo?cmd=listarCategoria&categoria=<%= categoria %>"
																class="px-4 py-2 block w-full links cursor-pointer">
																<%= categoria %>
															</a>
														</li>
														<% } %>
									...
									<li><a href="index.jsp#destaques" class="nav-link font-semibold links">Destaques</a>
									</li>
									<li><a href="jsp/team.jsp" class="nav-link font-semibold links">Sobre nós</a></li>
								</ul>
							</div>
							<div>
						...
						<% String categoria=request.getParameter("categoria"); BrinquedoDAO dao=new BrinquedoDAO();
							List<Brinquedo> brinquedosList = dao.listarPorCategoria(categoria);
							if (brinquedosList.isEmpty()) {
							%>
							<div class="flex justify-center items-center h-screen">
								<div style="border: 0.2px solid var(--third);"
									class="rounded-lg bg-white shadow-lg p-8 max-w-md text-center">
									<img class="mx-auto w-70 h-70" src="imgs/animation.gif" alt="Animação" />
									<p style="color: var(--third);" class="text-2xl font-semibold mb-8">Ops! Não temos
										brinquedos na categoria <span style="color: var(--fifth);">
											<%=categoria%>
										</span> ainda.</p>
									<p style="color: var(--complement2);" class="">Estamos trabalhando para adicionar
										mais brinquedos a esta categoria em breve.</p>
									<p style="color: var(--complement2);" class="mb-8">Por favor, aguarde.</p>
								</div>
							</div>
							<% } else { %>
									...
									<div>
										<div class="flex gap-4">
											<input style="background-color: var(--complement1); color: var(--third);"
												type="text" id="valorMin" placeholder="Valor Mínimo"
												class="px-3 py-2 border border-purple-600 rounded-full focus:outline-none focus:border-yellow-500"
												oninput="formatarValorMin(event)">
											<input style="background-color: var(--complement1); color: var(--third);"
												type="text" id="valorMax" placeholder="Valor Máximo"
												class="px-3 py-2 border border-purple-600  rounded-full focus:outline-none focus:border-yellow-500"
												oninput="formatarValorMax(event)">
										</div>
										<div class="flex gap-4 mt-4 justify-center items-center">
											<button
												class="buttons px-4 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 focus:outline-none"
												onclick="filtrarPorValor()">Filtrar</button>
											<button
												class="buttons px-4 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 focus:outline-none"
								...
								<div style="display: flex; flex-wrap: wrap;" class="justify-center">
									<% for (Brinquedo brinquedo : brinquedosList) { %>
										<div style="background-color: var(--fourth); min-width: 250px; margin: 10px;"
											class="w-64 p-3 border rounded-2xl brinquedo-card"
											data-valor="<%=brinquedo.getValor()%>">
											<img src="<%=brinquedo.getImage()%>" alt="<%=brinquedo.getNome()%>"
												class="w-full h-32 bg-white object-contain mx-auto rounded-xl" />
											<h3 style="color: var(--complement1);" class="text-lg font-semibold mt-4">
												<%=brinquedo.getNome()%>
											</h3>
								...
								<% } %>
									...
									<footer class=" bottom-0 w-full" style="background-color: var(--third);">
											...
											<div class="flex-col">
												<div class="flex gap-x-2">
													...
													<svg width="21" height="21" viewBox="0 0 21 21" fill="none"
														...
														<path
															d="M3 3.9375C3 3.41973 3.41973 3 3.9375 3H5.95582C6.41411 3 6.80522 3.33132 6.88056 3.78338L7.57367 7.94201C7.64133 8.34797 7.4363 8.7506 7.06819 8.93466L5.61681 9.66035C6.66334 12.2609 8.73907 14.3367 11.3397 15.3832L12.0653 13.9318C12.2494 13.5637 12.652 13.3587 13.058 13.4263L17.2166 14.1194C17.6687 14.1948 18 14.5859 18 15.0442V17.0625C18 17.5803 17.5803 18 17.0625 18H15.1875C8.45653 18 3 12.5435 3 5.8125V3.9375Z"
															fill="#F4F4F4" />
													</svg>
												</div>
												...
												<div class="flex gap-x-2 ">
													...
													<svg width="21" height="21" viewBox="0 0 21 21" fill="none"
														...
														<path fill-rule="evenodd" clip-rule="evenodd"
															d="M6.05025 4.05025C8.78392 1.31658 13.2161 1.31658 15.9497 4.05025C18.6834 6.78392 18.6834 11.2161 15.9497 13.9497L11 18.8995L6.05025 13.9497C3.31658 11.2161 3.31658 6.78392 6.05025 4.05025ZM11 11C12.1046 11 13 10.1046 13 9C13 7.89543 12.1046 7 11 7C9.89543 7 9 7.89543 9 9C9 10.1046 9.89543 11 11 11Z"
															fill="#F4F4F4" />
													</svg>
												</div>
										...
										<div style="background-color: var(--fifth); color: var(--third);"
											class="p-2 w-full font-medium flex justify-center items-center">
