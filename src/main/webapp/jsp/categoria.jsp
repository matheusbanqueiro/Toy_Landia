 ...
 <nav style="background-color: var(--complement1);"
    ...
    <div class="flex justify-center items-center h-screen">
        <div style="border: 0.2px solid var(--third);" class="rounded-lg bg-white shadow-lg p-8 max-w-md text-center">
            ...
            <p style="color: var(--third);" class="text-2xl font-semibold mb-8">Ops! Não temos brinquedos na categoria <span style="color: var(--fifth);"><%=categoria%></span> ainda.</p>
            <p style="color: var(--complement2);" class="">Estamos trabalhando para adicionar mais brinquedos a esta categoria em breve.</p>
    ...
    %>
    <h1 style="color: var(--third);" class="text-center text-2xl font-semibold mb-4"><%=categoria%></h1>
    <div class="flex justify-center space-x-4 mb-4">
    <div>
        <div class="flex gap-4">
            <input style="background-color: var(--complement1); color: var(--third);" type="text" id="valorMin" placeholder="Valor Mínimo" class="px-3 py-2 border border-purple-600 rounded-full focus:outline-none focus:border-yellow-500" oninput="formatarValorMin(event)">
            <input style="background-color: var(--complement1); color: var(--third);" type="text" id="valorMax" placeholder="Valor Máximo" class="px-3 py-2 border border-purple-600  rounded-full focus:outline-none focus:border-yellow-500" oninput="formatarValorMax(event)">
        </div>
        <div class="flex gap-4 mt-4 justify-center items-center">
        	<button class="buttons px-4 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 focus:outline-none" onclick="filtrarPorValor()">Filtrar</button>
			<button class="buttons px-4 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 focus:outline-none" onclick="limparPagina()">Limpar</button>    </div>
   	 	</div>
   	 	</div>
    <div style="display: flex; flex-wrap: wrap;" class="justify-center">
        ...
        %>
        <div style="background-color: var(--fourth); min-width: 250px; margin: 10px;" class="w-64 p-3 border rounded-2xl brinquedo-card" data-valor="<%=brinquedo.getValor()%>">
            <img src="<%=brinquedo.getImage()%>" alt="<%=brinquedo.getNome()%>" class="w-full h-32 bg-white object-contain mx-auto rounded-xl" />
            <h3 style="color: var(--complement1);" class="text-lg font-semibold mt-4"><%=brinquedo.getNome()%></h3>
            <p style="color: var(--complement1);" class="text-md"><%=brinquedo.getMarca()%></p>
            ...
            <% 
	    		...
	    		%>
	        	 ...
	        	 <%
                  ...
                  %>
            <a href="ServletBrinquedo?cmd=con&codigo=<%=brinquedo.getCodigo()%>" class="text-white flex justify-center py-2 px-2 rounded-full mt-4 text-center buttons">Ver mais</a>
        </div>
        <%
            }
        %>
