...
<body style="background-color: var(--complement1);" >
	...
	<nav style="background-color: var(--complement1);"
		...
		<a  >
			<img  class="h-auto w-14 ml-4 py-4 cursor-pointer" src="imgs/toylandia1.svg" alt="Logo" />
		</a>
		<div>
			<ul class="flex space-x-6">
				<li><a class="nav-link font-semibold links cursor-pointer">Início</a></li>
				...
				<li><a 
					class="nav-link font-semibold links cursor-pointer">Destaques</a></li>
				<li><a 
					class="nav-link font-semibold links cursor-pointer">Sobre nós</a></li>
			</ul>
		</div>
		<div>
	...
	<div class="flex flex-col justify-center items-center p-1 w-full bg-red-300">
    	<h1 class="text-lg font-semibold text-white">Estamos no preview do Brinquedo "<%= brinquedo.getNome() %>"</h1>
	</div>
   ...
   <div class="container mx-auto mt-8 mb-8">
    <div class="flex justify-center items-center">
        ...
        <div class="p-4">
            <img src="<%=brinquedo.getImage()%>" alt="<%=brinquedo.getNome()%>" class="w-80 h-80 rounded-xl">
        </div>

	        <div class="">
	       
	            <h1 style="color: var(--third);" class="text-3xl font-semibold mb-2"><%=brinquedo.getNome()%></h1>
	            <p style="color: var(--fourth);" class=" text-sm mb-10"><%=brinquedo.getMarca()%></p>
	        	...
	        	<% 
                 ...
                 %>
                 <div title="Brinquedo com desconto" class="mb-4 cursor-pointer inline-flex gap-4 p-2 rounded-r-lg shadow-lg justify-center items-center bg-green-600">
						    ...
						    <p class="text-white font-semibold text-sm">Produto com desconto</p>
						</div>
                 
                 	<p style="color: var(--fourth);" class="text-sm font-semibold line-through">De: R$ <%=brinquedo.getValor()%> reais</p>                
	        		<p style="color: var(--fifth);" class="text-2xl font-semibold ">Por: R$ <%=valorFinal%> reais à vista</p>
	        	 <%
                    } else {
                      %>
                         <p style="color: var(--fifth);" class="text-2xl font-semibold "> R$ <%=brinquedo.getValor() %> reais</p>
                      <%
                     }
                  %>
	        </div>
	    ...
	    <div class="mt-8">
    <h2 style="color: var(--third);" class="text-2xl font-semibold mb-4">Especificações</h2>
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <table class="min-w-full divide-y divide-gray-100">
            <tbody style="background-color: var(--third);"  class="bg-white divide-y divide-gray-100">
                <tr>
                    <td style="color: var(--primary);" class="px-6 py-4 w-1/4 font-semibold text-gray-600">
                        Descrição
                    </td>
                    <td class="px-6 py-4">
                        <div  class="description text-white">
                            <% String descricao = brinquedo.getDescricao();
                               int maxLength = 100; // Ajuste esse valor para controlar o comprimento máximo
                               if (descricao.length() > maxLength) {
                                   %>
                                   <p><%= descricao.substring(0, maxLength) %><span id="readMore" style="display:none;"><%= descricao.substring(maxLength) %></span></p>
                                   <a style="color: var(--primary);" href="javascript:void(0);" onclick="showDescription()">... Ler mais</a>
                                   <a style="color: var(--primary);  display:none;" href="javascript:void(0);" onclick="hideDescription()">... Ler menos</a>
                               <%
                               } else {
                                   %>
                                   <p><%= descricao %></p>
                ...
                <tr>
                    <td style="color: var(--primary);" class="px-6 py-4 w-1/4 font-semibold text-white">
                        Categoria
                    </td>
                ...
                <tr>
                    <td style="color: var(--primary);" class="px-6 py-4 w-1/4 font-semibold ">
                        Código
                    </td>
                    <td class="px-6 py-4 text-white">
                        <%= brinquedo.getCodigo() %>
   ...
   <h2 style="color: var(--third);" class="mt-10 text-2xl font-semibold mb-4">Card</h2>
    ...
    <div style="background-color: var(--fourth); min-width: 250px;" class="w-64 p-3 border rounded-2xl">
	   ...
	   <% 
	        		float valorDesconto2 = brinquedo.getValor();
                  	
                  	if (valorDesconto2 >= 200 ){
                  		float valorFinal = valorDesconto2 - 100;
	    ...
	    <a 
	        class="cursor-pointer text-white flex justify-center py-2 px-2 rounded-full mt-4 text-center buttons">
	        Ver mais
	    </a>
	</div>
</div>
	...
	<div class="flex flex-col justify-center items-center gap-4 p-2 w-full bg-red-700">
    <h1 class="text-lg font-bold text-white">Estamos no preview do Brinquedo "<%= brinquedo.getNome() %>"</h1>
    <a href="jsp/admin.jsp#listagem" class="text-white buttons py-2 px-4 rounded-lg">Voltar à administração</a>
</div>
     ...
     <footer class="  bottom-0 w-full " style="background-color: var(--third);">
		<div class="flex gap-12 justify-center items-center p-8">
			...
			<a >
				<img  class="cursor-pointer h-auto w-20 ml-4" src="imgs/toylandia1.png" alt="Logo" />
			</a>
		</div>
	...
	<script src="js/brinquedo/showDescription.js"></script>
</body>
